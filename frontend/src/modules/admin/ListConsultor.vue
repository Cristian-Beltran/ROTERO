<template>
  <LoadingBase v-if="consultorStore.loading" />
  <div v-else>
    <div class="mt-2">
      <Input class="w-3/12" v-model="consultorStore.search" placeholder="Buscar" />
    </div>
    <TableBase
      :items="consultorStore.filter"
      :columns="columns"
      :options="options"
      @action="action"
    />
...
const action = (data) => {
  if (data.action === 'edit') {
    router.push({ name: 'consultor-edit', query: { id: data.id } })
  } else if (data.action === 'delete') {
    swal({
      title: '¿Estas seguro?',
      text: 'Deshabilitarás al consultor',
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        consultorStore.deleteUser(data.id)
        consultorStore.getUsers()
      }
    })
  }
